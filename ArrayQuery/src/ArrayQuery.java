...
public class ArrayQuery {
	...
	public static List<Document> GetShippingByCity(String connectionString, String[] cities)
	{
		...
		try(MongoClient client = new MongoClient(clientUri))
		{
			...
			List<Document> queryResult = collection.find(in("shippingAddress.city",cities)).projection(fields(include("subtotal","shipping","shippingAddress.city"))).into(new ArrayList<Document>());
	...
	public List<Document> GetShippingByItem(String connectionString,String[] items)
	{
		...
		try(MongoClient client = new MongoClient(clientUri))
		{
			...
			List<Document> queryResult = collection.find(all("lineitems.name",items)).projection(fields(include("subtotal","lineitems.name","shippingAddress.city"))).into(new ArrayList<Document>());
			return queryResult;
		}
		catch (Exception e) {
			//log the exception
	...
	public static List<Document> FindByStatus(String connectionString, String status)
	{
		...
		try(MongoClient client = new MongoClient(clientUri))
		{
			...
			List<Document> queryResult = collection.find(eq("status",status)).into(new ArrayList<Document>());
	...
	public List<Document> FindByItemQuantity(String connectionString, String item_name, int quantity)
	{
		...
		try(MongoClient client = new MongoClient(clientUri))
		{
			...
			List<Document> queryResult = collection.find(elemMatch("lineitems",and(eq("name",item_name),gt("quantity",quantity)))).into(new ArrayList<Document>());
